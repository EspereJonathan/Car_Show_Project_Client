...
export default (apiUrl: string, httpClient = fetchUtils.fetchJson) => {
  /**
     ...
     */
  const convertDataRequestToHTTP = (type: string, resource: string, params: DataRequestParams): HTTPRequestParams => {
    ...
    switch (type) {
      ...
      case GET_MANY: {
        ...
        let idStr = "";
        const queryString = params.ids? params.ids.map(id => idStr + `id=${id}`) : [];
  ...
  /**
     ...
     */
  const convertHTTPResponse = async (response: Response, type: string, resource: string, params: DataRequestParams): Promise<ResponseData> => {
    const data = await response.json(); // Await the resolution of the promise
    switch (type) {
        case GET_LIST:
        case GET_MANY_REFERENCE:
            if (!data.hasOwnProperty("totalElements")) {
                throw new Error("The numberOfElements property must be must be present in the Json response");
            }
            return {
                data: data.content,
  ...
  /**
     ...
     */
  return (type: string, resource: string, params: DataRequestParams) => {
    ...
    return httpClient(url, options).then((responseObj) => {
    const statusTextMap: Record<number, string> = {
        200: 'OK',
        404: 'NotFound',
    };  
    const statusCode = responseObj.status;
    const statusText = statusTextMap[statusCode] || `Unknown (${statusCode})`;
